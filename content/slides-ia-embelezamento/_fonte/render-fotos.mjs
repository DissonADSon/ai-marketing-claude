...
const OUT = 'content/slides-ia-embelezamento-fotos';
mkdirSync(OUT, { recursive: true });

const browser = await chromium.launch({ args: ['--font-render-hinting=none'] });
const page = await browser.newPage({ viewport: { width: 1080, height: 1350 }, deviceScaleFactor: 1 });
await page.goto('file:///home/user/ai-marketing-claude/content/slides-ia-embelezamento/_fonte/slides-fotos.html', { waitUntil: 'networkidle' });
...
for (let i = 1; i <= 10; i++) {
  ...
  // overflow check: does content exceed the artboard?
  const r = await el.evaluate(e => {
    const of = e.scrollHeight - e.clientHeight;
    // a headline line that wrapped renders ~2x its line-height
    ...
    // largest vertical void between stacked children
    const kids = [...e.children].filter(c => !c.className.match(/num|swipe|art/) && c.getBoundingClientRect().height);
    let void_ = 0;
